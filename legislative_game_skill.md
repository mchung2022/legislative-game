...
#### 🟢 第一關：一讀提案與遊說連署 (Lobby & Signature Game)
- **機制**：玩家在 35 秒時限內，遊說立法院大廳的 15 位立委取得連署。點擊立委座位卡片會開啟對話框，玩家必須針對立委關注的群體（學生、家長、教師）與當前法案，選出合適的政策答辯。答對者立委立即簽名；答錯者則立委產生疑慮，卡片顯示進度條，需快速連點該立委卡片 5 次以進行說服化解。
- **擴充與修改技巧**：
  - 若需更改立委陣容或頭像，可直接編輯 [app.js](file:///c:/Users/DELL/Desktop/新增資料夾%20(2)/app.js) 中的 `legislatorsData`。
  - 若需修改特定法案下立委的遊說對話或選項，可修改 `getLegislatorDialogue()` 中的對話資料庫。
  - 若需調整連署的人數門檻或倒數秒數，可調整 `initStage1()` 中 `gameState.sigTimeLeft` 與 `signaturesCollected` 的判定標準。
